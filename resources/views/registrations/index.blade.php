...
<body>
    ...
    @if ($registration->qr_code)
        ...
        @if(file_exists($filePath))
            @if($extension == 'svg' || (file_exists($filePath) && strpos(file_get_contents($filePath), '<svg') !== false))
                <!-- It's an SVG file -->
                <object data="{{ asset('storage/' . $registration->qr_code) }}" 
                        type="image/svg+xml" 
                        class="qr-thumbnail"
                        style="width: 50px; height: 50px; cursor: pointer;"
                        onclick="document.getElementById('qrModal{{ $registration->id }}').classList.add('show'); document.getElementById('qrModal{{ $registration->id }}').style.display='block';">
                    <img src="{{ asset('images/qr-placeholder.png') }}" class="qr-thumbnail" alt="QR">
                </object>
            @else
                <!-- It's a PNG file -->
